...
@Component({
  ...
  styleUrls: ['./date-picker-ui.component.css']
})
export class DatePickerUiComponent implements OnInit {
...
onChange(monthIndex: number, year: number, date: number) {
 ...
 let number = this.model.dates[this.model.selectedMonthIndex]
  ...
  selectDateRange(date: number) {
    if( this.model.dateRange.start === -1) {
      this.model.dateRange.start =  date;
    }
    else {
      this.model.dateRange.end =  date;
    }
   }
   isSelectedDateRangeStart(date: number) {
    return  this.model.dateRange.start == date  && this.model.selectedMonthYear == this.model.selectedYearText;
  }
  isSelectedDateRangeEnd(date: number) {
    return  this.model.dateRange.end == date  && this.model.selectedMonthYear == this.model.selectedYearText;
  }
}
